...
class ConsultasFrame(ttk.Frame):
    ...
    def crear_interfaz(self):
        ...
        self.combo_consultas['values'] = (
            ...
            "2. Consumibles bajo stock mínimo de alerta",
            "3. Buscar herramientas por estado",
        ...
        # --- MODIFICACIÓN 1: Autocarga de la tabla al inicio ---
        # Fuerza a que la tabla se cargue con la consulta seleccionada por defecto
        # al abrir el módulo, evitando el cuadro gris vacío.
        self.ejecutar_consulta()

    def on_consulta_seleccionada(self, event):
        """Re-dibuja los campos de entrada y actualiza la tabla al instante."""
        for widget in self.inputs_frame.winfo_children():
            widget.destroy()
        
        self.generar_inputs_dinamicos()
        
        # --- MODIFICACIÓN 2: Sincronización instantánea al cambiar ---
        # En lugar de solo limpiar la tabla (self.limpiar_tabla()), 
        # ejecutamos la nueva consulta de inmediato para actualizar encabezados y datos.
        self.ejecutar_consulta()
    ...
    def mostrar_todo_inventario(self):
        ...
        if not documentos:
            # En lugar de un popup, simplemente dejamos la tabla vacía con sus encabezados
            # para no molestar al usuario en la carga inicial
            return
